...
var membaPBlokOBJ , membaPTextYStruct, membaPYKCizgi;
var membaPBlok     = new THREE.Group();

var mansapPBlokOBJ , mansapPTextYStruct, mansapPYKCizgi;
var mansapPBlok     = new THREE.Group();
...
function initializeGL(canvas , eventSource) {
    ...
    var palpColor = 0x010101;
    var px1 = 5, py1 = 90;
    var mPBParams           = ["P" , 0 , py1 , px1 , py1 , px1 , 0 ]
    var mPBCizgiSayi        = 3;
    var mPBKonum            = new THREE.Vector3(meshPosX,meshPosY - py1,0);
    var mPBFrame            = false;

    membaPBlokOBJ           = new BlokObje(mPBParams,mPBCizgiSayi,palpColor,
                                           mPBKonum,mPBFrame,0,0,membaPBlok);
    yeniBlokObje(membaPBlokOBJ);

    var mPBTextYAdi         = "MembaPalplanşText"
    var mPBTextY            = "9m";
    var mPBTextYPosition    = new THREE.Vector3(mPBKonum.x - 10 ,meshPosY - py1 / 2 + 5 , 100);
    var mPBTextYRotation    = new THREE.Vector3(0, 0 , 90);
    var mPBTextYOnRenk      = 0xffffff;
    var mPBTextYYanRenk     = 0x000000;
    membaPTextYStruct          = new TextStruct(mPBTextYAdi,mPBTextY, undefined/*daha mesh yok*/ ,mPBTextYPosition ,mPBTextYRotation,mPBTextYYanRenk,mPBTextYOnRenk,membaPBlok)
    newCreateText(membaPTextYStruct);

    var mPBYCizgiAdi        = "MembaPalplanşKonumÇizgi";
    var mPBYKCizgiPos       = new THREE.Vector3(mPBKonum.x - 5,meshPosY  - py1 / 2 /*offset*/ , 100)
    var mPBYKCizgiUzunluk   = 90;
    var mPBYKCizgiRotation  = new THREE.Vector3(0, 0 , 0);
    var mPBYKCizgiRenk      = 0xff0000;
    var mPBYKCizgiSize      = 0.65;
    var mPBYKCizgiArrowRenk = 0xff00ff
    membaPYKCizgi           = new KilavuzCizgi(mPBYCizgiAdi,mPBYKCizgiPos,mPBYKCizgiUzunluk,
                                               mPBYKCizgiRotation,mPBYKCizgiRenk,mPBYKCizgiSize,mPBYKCizgiArrowRenk,membaPBlok);

    objeGroup.add(membaPBlok);
    ...
    var manPBParams           = ["P" , 0 , py1 , px1 , py1 , px1 , 0 ]
    var manPBCizgiSayi        = 3;
    var manPBKonum            = new THREE.Vector3(sagIBKonum.x + x3 - px1,meshPosY - py1,0);
    var manPBFrame            = false;

    mansapPBlokOBJ           = new BlokObje(manPBParams,manPBCizgiSayi,palpColor,
                                           manPBKonum,manPBFrame,0,0,mansapPBlok);
    yeniBlokObje(mansapPBlokOBJ);

    var manPBTextYAdi         = "MansapPalplanşText"
    var manPBTextY            = "9m";
    var manPBTextYPosition    = new THREE.Vector3(manPBKonum.x + px1 + 10 ,meshPosY - py1 / 2 + 5 , 100);
    var manPBTextYRotation    = new THREE.Vector3(0, 0 , 90);
    var manPBTextYOnRenk      = 0xffffff;
    var manPBTextYYanRenk     = 0x000000;
    mansapPTextYStruct          = new TextStruct(manPBTextYAdi,manPBTextY, undefined/*daha mesh yok*/ ,manPBTextYPosition ,manPBTextYRotation,manPBTextYYanRenk,manPBTextYOnRenk,mansapPBlok)
    newCreateText(mansapPTextYStruct);

    var manPBYCizgiAdi        = "MansapPalplanşKonumÇizgi";
    var manPBYKCizgiPos       = new THREE.Vector3(manPBKonum.x + px1 + 5,meshPosY  - py1 / 2 /*offset*/ , 100)
    var manPBYKCizgiUzunluk   = 90;
    var manPBYKCizgiRotation  = new THREE.Vector3(0, 0 , 0);
    var manPBYKCizgiRenk      = 0xff0000;
    var manPBYKCizgiSize      = 0.65;
    var manPBYKCizgiArrowRenk = 0xff00ff
    mansapPYKCizgi           = new KilavuzCizgi(manPBYCizgiAdi,manPBYKCizgiPos,manPBYKCizgiUzunluk,
                                               manPBYKCizgiRotation,manPBYKCizgiRenk,manPBYKCizgiSize,manPBYKCizgiArrowRenk,mansapPBlok);

    objeGroup.add(mansapPBlok);
...
function changeBoyutOBJ(hangiNesne,newBoyut,Sonuc){
    ...
    case "SIBX1":
        ...
        {
            if(objeGroup.children[i].name === "P") continue;
    ...
    case "SAGIBX":
        ...
        for(var i = 5; i <objeGroup.children.length;i++)
        {
            if(objeGroup.children[i].name === "P") continue;
            objeGroup.children[i].position.x             += eklenecekFark;
        }
    ...
    case "palp": //a222
        console.log("Palplanaşa girdim = " + newBoyut);
        //Kılavuz Çizgi
        membaPYKCizgi.uzunluk             = parseFloat(newBoyut) * 10
        membaPYKCizgi.konum.y             = meshPosY - parseFloat(newBoyut) * 10 / 2
        changeKilavuzCizgi(membaPYKCizgi);
        //-----------

        //TextSturct--------
        membaPTextYStruct.text           = newBoyut + "m";
        membaPTextYStruct.textPosition.y = meshPosY - parseFloat(newBoyut) * 10 / 2 + 5
        changeText(membaPTextYStruct);
        //TextSturct--------

        //Kılavuz Çizgi
        mansapPYKCizgi.uzunluk             = parseFloat(newBoyut) * 10
        mansapPYKCizgi.konum.y             = meshPosY - parseFloat(newBoyut) * 10 / 2
        changeKilavuzCizgi(mansapPYKCizgi);
        //-----------

        //TextSturct--------
        mansapPTextYStruct.text           = newBoyut + "m";
        mansapPTextYStruct.textPosition.y = meshPosY - parseFloat(newBoyut) * 10 / 2 + 5
        changeText(mansapPTextYStruct);
        //TextSturct--------
